...
TEST(TypeMap_Tests, Assure_FirstTime_DefaultState)
{
  ...
  EXPECT_EQ(map.Assure<TestKey<0>>().value, 0u);
}
...
TEST(TypeMap_Tests, Assure_NonDefaultConstructibleFirstTime_DefaultState)
{
  ...
  EXPECT_EQ(map.Assure<TestKey<0>>(99LU).value_, 99u);
}
...
TEST(TypeMap_Tests, Assure_AfterStore_StoredValue)
{
  ...
  EXPECT_EQ(map.Assure<TestKey<0>>().value, 10u);
}
...
TEST(TypeMap_Tests, Assure_AfterStoreOfDifferentType_DefaultState)
{
  ...
  EXPECT_EQ(map.Assure<TestKey<1>>().value, 0u);
}
...
TEST(TypeMap_Tests, Assure_MultipleSets_StoredValues)
{
  ...
  EXPECT_EQ(map.Assure<TestKey<0>>().value, 0u);
  EXPECT_EQ(map.Assure<TestKey<1>>().value, 1u);
  EXPECT_EQ(map.Assure<TestKey<2>>().value, 2u);
  EXPECT_EQ(map.Assure<TestKey<3>>().value, 3u);
  EXPECT_EQ(map.Assure<TestKey<4>>().value, 4u);
  EXPECT_EQ(map.Assure<TestKey<5>>().value, 5u);
  EXPECT_EQ(map.Assure<TestKey<6>>().value, 6u);
  EXPECT_EQ(map.Assure<TestKey<7>>().value, 7u);
}
...
TEST(TypeMap_Tests, Get_AfterAssureStore_SameAsStored)
{
  ...
  EXPECT_EQ(map.Get<TestKey<0>>().value, 10u);
}
...
TEST(TypeMap_Tests, Get_AfterAssureStoreOfDifferentType_Zero)
{
  ...
  EXPECT_EQ(map.Get<TestKey<1>>().value, 0u);
}
